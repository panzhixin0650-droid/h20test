#!/usr/bin/env bash
# DLC single-entry two-node benchmark: true GQLA cache + HPC-Ops decode,
# TP8/PP2, exact 16K input and 128-token output. Submit this same entrypoint
# to a two-worker DLC job with 8 visible GPUs per worker; the shared launcher
# automatically resolves head versus headless worker from DLC rank variables.
...
export SERVED_MODEL_NAME=dsv3p1-g8-gqla-h100-tp8-pp2-16k
export OUTPUT_ROOT=${OUTPUT_ROOT:-$GQLA_ROOT/outputs/benchmarks/dsv3p1_g8_gqla_h100_tp8_pp2_16k}
export RUN_ID=${RUN_ID:-h100-gqla-tp8-pp2-16k-001}
